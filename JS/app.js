...
function showCartProducts() {
    ...
    cartProducts.forEach(product => {
        ...
        removeBtn.addEventListener("click", () => {
            const produse = getCartProducts().filter(p => p.id !== product.id);
            saveCart(produse);
            showCartProducts();
            // Reset card button if visible
        ...
        plusBtn.addEventListener("click", () => {
            ...
            const found = produse.find(p => p.id === product.id);
            if (found) {
                found.quantity++;
                saveCart(produse);
                showCartProducts();
        ...
        minusBtn.addEventListener("click", () => {
            ...
            if (found) {
                found.quantity--;
                if (found.quantity <= 0) {
                    produse = produse.filter(p => p.id !== product.id);
                    document.querySelectorAll(".card").forEach(card => {
                        const title = card.querySelector("h3");
                        const btn = card.querySelector(".add-to-cartBtn");
                        if (title && btn && title.textContent === product.name) {
